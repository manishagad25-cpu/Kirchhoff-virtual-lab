...
export const allKtxFormats = ["-astc.ktx", "-dxt.ktx", "-pvrtc.ktx", "-etc1.ktx", "-etc2.ktx"];
export const ktxSupportedextensions = [".png", ".jpg", ".jpeg", ".bmp"];
/**
 ...
 */
export function getCompressedTextureFilename(path, format) {
    return `${path.substring(0, path.lastIndexOf("."))}${format}`;
}
export async function compressFileToKtx(absolutePath, options) {
    if (options.format) {
        await compressFileToKtxFormat(absolutePath, options);
    }
    else {
        await Promise.all(allKtxFormats.map((f) => compressFileToKtxFormat(absolutePath, {
            ...options,
            format: f,
        })));
    }
}
export async function compressFileToKtxFormat(absolutePath, options) {
