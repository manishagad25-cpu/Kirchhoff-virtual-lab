...
var WebRequest = class _WebRequest {
  ...
  static _CollectCustomizations(url, baseHeaders = {}) {
    ...
    if (_WebRequest._ShouldSkipRequestModifications(url)) {
      return { url, headers };
    }
    ...
    for (const modifier of _WebRequest.CustomRequestModifiers) {
      if (_WebRequest._ShouldSkipRequestModifications(url)) {
        break;
      }
  ...
  static async FetchAsync(url, options = {}) {
    ...
    if (typeof fetch !== "undefined") {
      const { url: resolvedUrl, headers } = _WebRequest._CollectCustomizations(_WebRequest._CleanUrl(url), options.headers ?? {});
      return await fetch(resolvedUrl, { method, headers, body: options.body ?? void 0 });
    }
    return await new Promise((resolve, reject) => {
      const request = new _WebRequest();
  ...
  open(method, url, baseHeaders) {
    const { url: modifiedUrl, headers } = _WebRequest._CollectCustomizations(url, baseHeaders);
    this._requestURL = _WebRequest._CleanUrl(modifiedUrl);
...
//# sourceMappingURL=chunk-A2S6EVW2.js.map
