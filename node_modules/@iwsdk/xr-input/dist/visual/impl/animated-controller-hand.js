import { Matrix4, Vector3, BufferAttribute, Quaternion } from 'three';
import { lerp } from '../../node_modules/.pnpm/super-three@0.181.0/node_modules/super-three/src/math/MathUtils.js';
import { stencilMaterial, outlineMaterial } from './animated-hand.js';
...
const pressedPose = {
    ...
    'ring-finger-tip': [
        ...
        -0.6362114548683167, 0.7475619316101074, 0.1907520294189453, 0,
        -0.7446783781051636, -0.5303730368614197, -0.40516555309295654, 0,
        0.011834603734314442, -3507334040477872e-19, 0.031390633434057236, 1,
    ],
    'pinky-finger-metacarpal': [
...
class AnimatedControllerHand extends BaseControllerVisual {
    constructor(model, scene, camera, layout) {
        ...
        for (let i = 0; i < vertexCount; i++) {
            ...
            distances[i] = Math.sqrt((x - wristLocal.x) * (x - wristLocal.x) +
        ...
        (_a = skinnedMesh.parent) === null || _a === void 0 ? void 0 : _a.add(skinnedOutline);
    ...
    connect(inputSource, enabled) {
        ...
        this.bones.forEach((bone) => {
            ...
            this.mat4.decompose(bone.userData.defaultTransform.position, bone.userData.defaultTransform.quaternion, this.vec3);
            this.mat4.fromArray(AnimatedControllerHand.pressedPose[jointName]);
            this.mat4.decompose(bone.userData.pressedTransform.position, bone.userData.pressedTransform.quaternion, this.vec3);
    ...
    update(delta) {
        if (this.gamepad && this.enabled) {
            this.bones.forEach((bone) => {
                if (bone) {
                    const { buttonIndex, defaultTransform, pressedTransform, alpha } = bone.userData;
                    const button = this.gamepad.buttons[buttonIndex];
                    const newAlpha = lerp(alpha, button.value, delta * 20);
                    bone.position.lerpVectors(defaultTransform.position, pressedTransform.position, newAlpha);
                    bone.quaternion.slerpQuaternions(defaultTransform.quaternion, pressedTransform.quaternion, newAlpha);
...
AnimatedControllerHand.assetKeyPrefix = 'controller-hand-';
AnimatedControllerHand.assetProfileId = 'generic-hand';
AnimatedControllerHand.defaultPose = defaultPose;
AnimatedControllerHand.pressedPose = pressedPose;

export { AnimatedControllerHand };
//# sourceMappingURL=animated-controller-hand.js.map
